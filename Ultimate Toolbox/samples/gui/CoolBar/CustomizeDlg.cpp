...
void CCustomizeDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_IMAGE, m_ctlComboImage);
	...
	DDX_Text(pDX, IDC_EDIT_BAND_TEXT, m_sBandText);
	DDX_CBIndex(pDX, IDC_COMBO_IMAGE, m_nImage);
	//}}AFX_DATA_MAP
}
...
BOOL CCustomizeDlg::OnInitDialog() 
{
	...
	// names of images
	m_ctlComboImage.AddString(_T("File"));
	m_ctlComboImage.AddString(_T("Edit"));
	m_ctlComboImage.AddString(_T("Window"));
	m_ctlComboImage.AddString(_T("None"));
...
void CCustomizeDlg::GetVars()
{
	...
	m_nImage=arrCustomize[m_nToolbar].nImage;
	m_sBandText=arrCustomize[m_nToolbar].sBandText;
...
void CCustomizeDlg::SetVars()
{
	...
	arrCustomize[m_nOldToolbar].nImage=m_nImage;
	arrCustomize[m_nOldToolbar].sBandText=m_sBandText;

	if(arrCustomize[m_nOldToolbar].nImage==3)
	{
		arrCustomize[m_nOldToolbar].nImage=-1;
	}
